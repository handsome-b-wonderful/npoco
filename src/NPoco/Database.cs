    ...
    public class Database : IDatabase
    {
        ...
        public virtual void AddParameter(IDbCommand cmd, object value)
        {
            ...
            else
            {
                ...
                if (t == typeof(byte[]) && ((byte[])value).Length == 0)
                    p.Value = DBNull.Value; // HACK: set value of empty byte array to DbNull
            }
        ...
        {
            if (Mapper != null)
                if (!Mapper.OnInserting(this, new InsertContext(poco, tableName, autoIncrement, primaryKeyName))) return 0;
            try
            {
                OpenSharedConnectionInternal();
                ...
                using (var cmd = CreateCommand(_sharedConnection, sql, rawvalues.ToArray()))
                {
                    ...
                    if (!autoIncrement)
                    {
                        ...
                        if (Mapper != null)
                            Mapper.OnInserted(this, new InsertContext(poco, tableName, autoIncrement, primaryKeyName));
        ...
        {
            if (Mapper != null)
                if (!Mapper.OnUpdating(this, new UpdateContext(poco, tableName, primaryKeyName, primaryKeyValue, columns))) return 0;
            ...
            if (result != 0)
                 if (Mapper != null)
                    if (!Mapper.OnUpdated(this, (new UpdateContext(poco, tableName, primaryKeyName, primaryKeyValue, columns)))) return 0;
        ...
        {
            if (Mapper != null)
                if (!Mapper.OnDeleting(this, new DeleteContext(poco, tableName, primaryKeyName, primaryKeyValue))) return 0;
            ...
            var result = Execute(sql, primaryKeyValuePairs.Select(x => x.Value).ToArray());
            if (result != 0)
                if (Mapper != null)
                    if (!Mapper.OnDeleted(this, new DeleteContext(poco, tableName, primaryKeyName, primaryKeyValue))) return 0;
